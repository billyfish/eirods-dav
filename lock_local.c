/**
 * \file
 * \brief     Davrods locking support using a modified mod_dav_lock.
 * \author    Chris Smeele
 ...
 */
const dav_hooks_locks davrods_hooks_locallock;
		...
		apr_status_t status)
{
	...
	if (db == NULL)
		{
			errcode = 1;
			errstr = "Could not open lock database.";
		}
	else
		{
...
static dav_error * dav_generic_parse_locktoken (apr_pool_t *p,
		const char *char_token, dav_locktoken **locktoken_p)
{
	dav_locktoken *locktoken;
		...
		const dav_locktoken *lt2)
{
	uint64_t l1 [2] = { 0 };
	uint64_t l2 [2] = { 0 };
	memcpy (l1, lt1->uuid.data, 16);
	memcpy (l2, lt2->uuid.data, 16);
		...
		dav_lockdb **lockdb)
{
	...
	comb->pub.hooks = &davrods_hooks_locallock;
	comb->pub.ro = ro;
	comb->pub.info = &comb->priv;
	comb->priv.r = r;
			...
			if (comb->priv.lockdb_path == NULL)
				{
					...
					DAV_ERR_LOCK_NO_DB, 0, "A lock database was not specified with the "
		...
		const dav_resource *resource)
{
	...
	// @cjsmeele: Lock based on iRODS paths instead of HTTP URIs,
	// because URIs may point to different resources for different
	// users, depending on the DavrodsExposedRoot setting.
	const char *pathname = resource->info->rods_path;
...
static dav_error * dav_generic_save_lock_record (dav_lockdb *lockdb,
		apr_datum_t key, dav_lock_discovery *direct, dav_lock_indirect *indirect)
{
	dav_error *err;
	apr_status_t status;
	apr_datum_t val = { 0 };
	...
	if ((status = apr_dbm_store (lockdb->info->db, key, val)) != APR_SUCCESS)
		{
			...
			return dav_push_error (lockdb->info->pool,
			HTTP_INTERNAL_SERVER_ERROR,
			DAV_ERR_LOCK_SAVE_LOCK, "Could not save lock information.", err);
		}
...
static dav_error * dav_generic_load_lock_record (dav_lockdb *lockdb,
		apr_datum_t key, int add_method, dav_lock_discovery **direct,
	...
	while (offset < val.dsize)
		{
			...
			case DAV_LOCK_DIRECT:
				...
				/* Copy the lock token. */
				dp->locktoken = apr_pmemdup (p, val.dptr + offset,
						sizeof(*dp->locktoken));
			...
			case DAV_LOCK_INDIRECT:
				...
				ip->locktoken = apr_pmemdup (p, val.dptr + offset,
						sizeof(*ip->locktoken));
			...
			default:
				...
				DAV_ERR_LOCK_CORRUPT_DB, 0,
						apr_psprintf (p, "The lock database was found to "
								"be corrupt. offset %"
						APR_SIZE_T_FMT ", c=%02x", offset, val.dptr [offset]));
				}
		}
...
static dav_error * dav_generic_resolve (dav_lockdb *lockdb,
		dav_lock_indirect *indirect, dav_lock_discovery **direct,
		dav_lock_discovery **ref_dp, dav_lock_indirect **ref_ip)
{
	dav_error *err;
	...
	return dav_new_error (lockdb->info->pool,
	HTTP_INTERNAL_SERVER_ERROR,
	DAV_ERR_LOCK_CORRUPT_DB, 0, "The lock database was found to be corrupt. "
...
static dav_error * dav_generic_create_lock (dav_lockdb *lockdb,
		const dav_resource *resource, dav_lock **lock)
{
	apr_datum_t key;
...
static dav_error * dav_generic_get_locks (dav_lockdb *lockdb,
		const dav_resource *resource, int calltype, dav_lock **locks)
{
	apr_pool_t *p = lockdb->info->pool;
	apr_datum_t key;
	dav_error *err;
	...
	if ((err = dav_generic_load_lock_record (lockdb, key, DAV_CREATE_LIST, &dp,
			&ip)) != NULL)
		{
...
static dav_error * dav_generic_find_lock (dav_lockdb *lockdb,
		const dav_resource *resource, const dav_locktoken *locktoken,
...
static dav_error * dav_generic_has_locks (dav_lockdb *lockdb,
		const dav_resource *resource, int *locks_present)
{
	dav_error *err;
	apr_datum_t key;
...
static dav_error * dav_generic_refresh_locks (dav_lockdb *lockdb,
		const dav_resource *resource, const dav_locktoken_list *ltl,
	...
	for (; ip != NULL; ip = ip->next)
		{
			...
			if ((err = dav_generic_resolve (lockdb, ip, &dp_scan, &ref_dp, &ref_ip))
					!= NULL)
				{
					/* ### push a higher-level desc? */
					return err;
			...
			if (dav_generic_do_refresh (dp_scan, ltl, new_time))
				{
					/* the lock was refreshed. return the lock. */
					newlock = dav_generic_alloc_lock (lockdb, ip->key,
							dp_scan->locktoken);
					newlock->is_locknull = !resource->exists;
					newlock->scope = dp_scan->f.scope;
					newlock->type = dp_scan->f.type;
...
const dav_hooks_locks davrods_hooks_locallock = { dav_generic_get_supportedlock,
		dav_generic_parse_locktoken, dav_generic_format_locktoken,
		dav_generic_compare_locktoken, dav_generic_open_lockdb,
		dav_generic_close_lockdb, dav_generic_remove_locknull_state,
		dav_generic_create_lock, dav_generic_get_locks, dav_generic_find_lock,
		dav_generic_has_locks, dav_generic_append_locks, dav_generic_remove_lock,
 ...
 * \author Chris Smeele
 */
dav_error* davrods_locklocal_get_locked_entries (dav_lockdb *lockdb,
		const dav_resource *col, davrods_locklocal_lock_list_t **names)
{
	const char *colpath = col->info->rods_path;
	*names = NULL;
	davrods_locklocal_lock_list_t *curname = NULL;

	dav_error *err;
	if ((err = dav_generic_really_open_lockdb (lockdb)) != NULL)
		return err;

	apr_dbm_t *dbm = lockdb->info->db;

	apr_datum_t key = { 0 };
	//apr_datum_t value = { 0 };

	int apr_err = apr_dbm_firstkey (dbm, &key);

	for (; key.dsize; apr_err = apr_dbm_nextkey (dbm, &key))
		{
			if (apr_err)
				{
					if (apr_err == APR_EOF)
						...
						return dav_new_error (lockdb->info->pool,
								HTTP_INTERNAL_SERVER_ERROR, 0, apr_err,
								"Could not iterate DBM keys.");
				}

			const char *locked_path = key.dptr + 1;

			if (strstr (locked_path, colpath) == locked_path
					&& locked_path [strlen (colpath)] == '/'
					&& strrchr (locked_path, '/') == locked_path + strlen (colpath))
				{
					// The locked resource is a member of the given collection.

					davrods_locklocal_lock_list_t* llEntry = apr_palloc (lockdb->info->pool, sizeof(davrods_locklocal_lock_list_t));

					if (llEntry)
						{
							llEntry->entry = apr_pstrdup (lockdb->info->pool, locked_path);
							llEntry->next = NULL;

							// Append the item to our output lock list.
							if (curname)
								{
									curname->next = llEntry;
								}
							else
								{
									*names = llEntry;
